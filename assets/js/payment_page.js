let paymentForm = document.querySelector(".payment-gateway .container .payment-right form");
...
paymentForm.addEventListener("submit", (event) => {
    ...
    if (cardName.value === "" || cardNumberInput.value === "" || expiryInput.value === "" || cvvInput.value === "" || patternCvv.test(cardNumberInput)) {
      alert("Please Enter Information")
    
    } else if(patternCardNumber.test(cardNumberInput.value) || patternExpiryDate.test(expiryInput.value) || patternCvv.test(cvvInput.value)) {
        setTimeout(() => { 
          ShowCongratsPopUp();
...
function showFaildPopUp() {
  ...
  `;
  document.querySelector("header").insertAdjacentElement("afterend",faildPopup);
...
function ShowCongratsPopUp(){
  ...
  `;
  document.querySelector("header").insertAdjacentElement("afterend",congratspopup);
